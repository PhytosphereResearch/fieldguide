...
knitr::include_graphics("images/distributions/CAoakrange.png")
```
...
The included range maps are approximate because the distributions of many agents have not been fully documented. These ranges are also subject to expansion over time, especially in the case of introduced agents. For most agents, we have  used county boundaries to show the reported range. Agents may also occur beyond the illustrated ranges and are not necessarily found in all portions of the illustrated ranges.

To the degree possible, the taxonomy of agents has been updated to current accepted names. However, not every proposed name becomes accepted, and some names that have been changed later revert to former names. Legitimately published former names are considered synonyms. For some agents, we have included recent or more widely known synonyms using the equal sign, e.g., *Aceria* (=*Eriophyes*) *mackiei*. Additional common synonyms for many agents are listed on the agent page in the CODA database (https://coda.phytosphere.com).
...
Swiecki, T.J.; Bernhardt, E.A. 2025. Introduction to the Field Guide. In Swiecki, T.J., et al. (Eds.) Field Guide to Insects and Diseases of California Oaks, Online edition `r Sys.Date()`.